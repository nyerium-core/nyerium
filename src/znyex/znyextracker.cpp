...
#include "znyex/znyexwallet.h"


CzNYEXTracker::CzNYEXTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzNYEXTracker::~CzNYEXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNYEXTracker::Init()
{
...
bool CzNYEXTracker::Archive(CMintMeta& meta)
{
...
bool CzNYEXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNYEXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNYEXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNYEXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNYEXTracker::GetSerialHashes()
{
...
CAmount CzNYEXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNYEXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNYEXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNYEXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNYEXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNYEXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNYEXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNYEXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNYEXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNYEXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzNYEXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNYEXWallet* zNYEXWallet)
{
    bool iszNYEXWalletInitialized = (nullptr != zNYEXWallet);
    CMintMeta meta;
    ...
    if (!iszNYEXWalletInitialized)
        zNYEXWallet = new CzNYEXWallet(wallet);
    meta.isSeedCorrect = zNYEXWallet->CheckSeed(dMint);
    if (!iszNYEXWalletInitialized)
        delete zNYEXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNYEXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNYEXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNYEXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNYEXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNYEXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNYEXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dznyex from DB\n", __func__, listDeterministicDB.size());
    }
