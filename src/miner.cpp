...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    bool fZerocoinActive = nHeight >= consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight;
    if (fZerocoinActive)
        pblock->nVersion = 4;
    else
        pblock->nVersion = 3;
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zNYEX spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zNYEX spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zNYEX serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
{
    LogPrintf("%s\n", pblock->ToString());
    LogPrintf("miner.cpp L -----> 520 ProcessBlockFound generated %s\n", FormatMoney(pblock->vtx[0].vout[0].nValue));

    // Found a solution
    {
        WAIT_LOCK(g_best_block_mutex, lock);
        if (pblock->hashPrevBlock != g_best_block)
            return error("NyeriumMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("NyeriumMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("NyeriumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("nyerium-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running NyeriumMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("NyeriumMiner exception");
    } catch (...) {
        LogPrintf("NyeriumMiner exception");
    }

    LogPrintf("NyeriumMiner exiting\n");
}
