...
#include "qt/nyerium/forms/ui_snackbar.h"
#include "qt/nyerium/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(NyeriumGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &NyeriumGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
