...
#include "utilstrencodings.h"  // EK ADDED
...
CBlockHeader CBlockIndex::GetBlockHeader() const
{
    ...
    block.nBits = nBits;
    block.nNonce = nNonce;
    if (nVersion > 3) block.nAccumulatorCheckpoint = nAccumulatorCheckpoint;
    return block;
}
...
int64_t CBlockIndex::MinPastBlockTime() const
{
    ...
    // so we allow the nBlockTimeProtocolV2 (Nyerium v4.0) to be at most (180-14) seconds earlier than previous block
...
void CBlockIndex::SetStakeModifier(const uint256& nStakeModifier)
{
    ...
    //LogPrintf("chain.cpp CBlockIndex::SetStakeModifier v2 ==========================>: %s \n", HexStr(vStakeModifier)); 
...
uint64_t CBlockIndex::GetStakeModifierV1() const
{
    if (vStakeModifier.empty() || Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V3_4)){
        //LogPrintf("chain.cpp GetStakeModifierV1 ==========================> nheight %d ------- upgrade %d\n", nHeight, Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V3_4)); 

        return 0;
    }
    uint64_t nStakeModifier;
    std::memcpy(&nStakeModifier, vStakeModifier.data(), vStakeModifier.size());
    //LogPrintf("chain.cpp GetStakeModifierV1 ==========================>: %s \n", std::to_string(nStakeModifier).c_str()); 
...
uint256 CBlockIndex::GetStakeModifierV2() const
{
    ...
    //LogPrintf("chain.cpp GetStakeModifierV2 ==========================> nStakeModifier %s\n", nStakeModifier.ToString().c_str()); 
