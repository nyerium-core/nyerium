    ...
    stakeValue(stakeInput->GetValue())
{
    ...
    if (!Params().GetConsensus().NetworkUpgradeActive(pindexPrev->nHeight + 1, Consensus::UPGRADE_V3_4)) {
        ...
        //LogPrintf("kernel.cpp OLD StakeModifier ==========================>: %s ====> nStakeModifier: %s\n", HexStr(stakeModifier), std::to_string(nStakeModifier).c_str()); 
    } else {
        // Modifier v2
        stakeModifier << pindexPrev->GetStakeModifierV2();
        //LogPrintf("kernel.cpp NEW StakeModifier ==========================>: %s\n", HexStr(stakeModifier));
    }
...
uint256 CStakeKernel::GetHash() const
{
    ...
    //LogPrintf("kernel.cpp SS with StakeModifier ==========================>: %s\n", HexStr(ss.begin(), ss.end()).c_str());
    ss << nTimeBlockFrom << stakeUniqueness << nTime;
    //uint256 ekhash=XEVAN(ss.begin(), ss.end()); // TODO EK TRIAL BEFORE AND AFTER FORK
    //uint256 rhash=Hash(ss.begin(), ss.end()); // TODO EK TRIAL BEFORE AND AFTER FORK
    //LogPrintf("kernel.cpp SS with other data ==========================>: %s, ekhash =%s <==================> real hash=%s\n", HexStr(ss.begin(), ss.end()).c_str(), ekhash.GetHex(), rhash.GetHex());
...
bool LoadStakeInput(const CBlock& block, const CBlockIndex* pindexPrev, std::unique_ptr<CStakeInput>& stake)
{
    ...
    stake = txin.IsZerocoinSpend() ?
            std::unique_ptr<CStakeInput>(new CLegacyZNyexStake()) :
            std::unique_ptr<CStakeInput>(new CNyexStake());
...
bool CheckProofOfStake(const CBlock& block, std::string& strError, const CBlockIndex* pindexPrev)
{
    ...
    if (stakeInput->IsZNYEX()) return true;
