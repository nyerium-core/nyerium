...
#if defined(NDEBUG)
#error "Nyerium cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in unyex) are considered zero fee (for relaying, mining and transaction creation)
...
const std::string strMessageMagic = "Nyerium Signed Message:\n";
        ...
        bool* pfMissingInputs, bool fOverrideMempoolLimit, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zNYEX mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zNYEX mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 20000) { nSubsidy = 250 * COIN; }
	else if (nHeight <= 43200) { nSubsidy = 100 * COIN; }
	else if (nHeight <= 86400) { nSubsidy = 50 * COIN; }
	else if (nHeight <= 100000) { nSubsidy = 60 * COIN; } //Will study masternode collateral increase after this block
	else if (nHeight <= 129600) { nSubsidy = 40 * COIN; }
	else if (nHeight <= 172800) { nSubsidy = 30 * COIN; }
	else if (nHeight <= 216000) { nSubsidy = 20 * COIN; }
	else if (nHeight <= 259200) { nSubsidy = 15 * COIN; }
	else if (nHeight <= 302400) { nSubsidy = 10 * COIN; }
	else if (nHeight <= 518400) { nSubsidy = 5 * COIN; }
	else if (nHeight <= 525600) { nSubsidy = 100 * COIN; } //SUPER BLOCK (~7200 blocks OR 5 days 720,000 nyex)
	else if (nHeight <= 568800) { nSubsidy = 40 * COIN; }
	else if (nHeight <= 612000) { nSubsidy = 30 * COIN; }
	else if (nHeight <= 655200) { nSubsidy = 20 * COIN; }
	else if (nHeight <= 698400) { nSubsidy = 10 * COIN; }
	else if (nHeight <= 1049760) { nSubsidy = 5 * COIN; }
	else if (nHeight <= 1059760) { nSubsidy = 120 * COIN; } //SUPER BLOCK (~10,000 blocks OR ~6 days 1,200,000 nyex)
	else if (nHeight <= 1102960) { nSubsidy = 30 * COIN; }
	else if (nHeight <= 1146160) { nSubsidy = 20 * COIN; }
	else if (nHeight <= 1189360) { nSubsidy = 10 * COIN; }
	else if (nHeight <= 1534960) { nSubsidy = 5 * COIN; }
	else if (nHeight <= 1576720) { nSubsidy = 10 * COIN; }
	else if (nHeight <= 1586720) { nSubsidy = 150 * COIN; } //SUPER BLOCK (~10,000 blocks OR ~6 days 1,500,000 nyex)
	else if (nHeight <= 1629920) { nSubsidy = 25 * COIN; }
	else if (nHeight <= 1673120) { nSubsidy = 20 * COIN; }
	else if (nHeight <= 1716320) { nSubsidy = 15 * COIN; }
	else if (nHeight <= 1759520) { nSubsidy = 10 * COIN; }
	else if (nHeight <= 2018720) { nSubsidy = 5 * COIN; }
	else if (nHeight <= 2061920) { nSubsidy = 10 * COIN; }
	else if (nHeight <= 2102240) { nSubsidy = 15 * COIN; }
	else if (nHeight <= 2114240) { nSubsidy = 200 * COIN; } //SUPER BLOCK (~12,000 blocks OR ~6 days 2,400,000 nyex)
	else if (nHeight <= 2120240) { nSubsidy = 80 * COIN; }
	else if (nHeight <= 2163440) { nSubsidy = 40 * COIN; }
	else if (nHeight <= 2206640) { nSubsidy = 20 * COIN; }
	else if (nHeight <= 2249840) { nSubsidy = 10 * COIN; }
	else if (nHeight <= 2627120) { nSubsidy = 5 * COIN; }
	else if (nHeight <= 2630000) { nSubsidy = 220 * COIN; } //SUPER BLOCK (~2,880 blocks FOR 2 days 633,600 nyex)
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZNYEXModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    //Track zNYEX money supply
    if (!UpdateZNYEXSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zNYEX supply", __func__);
...
{
    util::ThreadRename("nyerium-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight > consensus.vUpgrades[Consensus::UPGRADE_POS].nActivationHeight && block.IsProofOfWork())
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZNYEXModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    // Update zNYEX money supply map
    if (!UpdateZNYEXSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zNYEX supply for block=%s height=%d", __func__,
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (!CheckBlockHeader(block, state, !IsPoS))
        return state.DoS(100, false, REJECT_INVALID, "CheckBlock() : CheckBlockHeader failed", true, "bad-header");
        //return false;
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zNYEX spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNYEX serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        //const Consensus::Params& consensus = Params().GetConsensus();
        //if ((block.nTime == (uint32_t) consensus.nNyeriumBadBlockTime) &&
        //        (block.nBits == (uint32_t) consensus.nNyeriumBadBlockBits)) {
            // accept Nyerium block minted with incorrect proof of work threshold
        //    return true;
        //}
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    // EK TODO. RESTORE THIS AT SOME POINT. BACKWARD COMPATIBILITY ISSUE
    if (isPoS) {
        std::string strError;
        //CheckProofOfStake(block, strError, pindexPrev);
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> nyexInputs;
        std::vector<CTxIn> zNYEXInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zNYEXInputs.push_back(stakeIn);
            }else{
                nyexInputs.push_back(stakeIn);
            }
        }
        const bool hasNYEXInputs = !nyexInputs.empty();
        const bool hasZNYEXInputs = !zNYEXInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasNYEXInputs) {
                    ...
                    for (const CTxIn& nyexIn : nyexInputs)
                        if(nyexIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zNYEX inputs.
            if(hasZNYEXInputs) {
                for (const CTxIn& zNyexInput : zNYEXInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zNyexInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-znyex");
