...
#include "ui_znyexcontroldialog.h"
...
std::list<std::string> ZNyexControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNyexControlDialog::listMints;

ZNyexControlDialog::ZNyexControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNyexControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNyexControlDialog::~ZNyexControlDialog()
{
    delete ui;
}

void ZNyexControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNyexControlDialog::updateList()
{
...
void ZNyexControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNyexControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNyex_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNyexControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNyexControlDialog::GetSelectedMints()
{
...
void ZNyexControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
