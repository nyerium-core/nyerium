...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    for (const CTxOut &o : txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 NYEX finalization collateral
            //       will also cover the old 50 NYEX finalization collateral.
...
{
	// Nyerium for now. Add below later
    CAmount nSubsidy = 0;
    ...
    totalBudget = nSubsidy  * .1 * Params().GetConsensus().nBudgetCycleBlocks;
