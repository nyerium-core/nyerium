...
bool CNyexStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CNyexStake from zerocoin spend", __func__);
...
bool CNyexStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CNyexStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CNyexStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CNyexStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CNyexStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CNyexStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CNyexStake::GetUniqueness() const
{
    //The unique identifier for a NYEX stake is the outpoint
...
CBlockIndex* CNyexStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CNyexStake::ContextCheck(int nHeight, uint32_t nTime)
{
