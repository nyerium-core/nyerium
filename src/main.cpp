...
const string strMessageMagic = "Nyerium Signed Message:\n";
...
// nyerium: total coin age spent in transaction, in the unit of coin-days.
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zNyex spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
					return state.Invalid(error("%s : zNyex spend with serial %s is already in block %d\n",
				...
				if (!spend.HasValidSerial(Params().Zerocoin_Params()))
					return state.Invalid(error("%s : zNyex spend with serial %s from tx %s is not in valid range\n",
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		 ...
		 * note we only undo zerocoin databasing in the following statement, value to and from Nyerium
...
{
	RenameThread("nyerium-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZNYEXMinted()
{
...
void RecalculateZNYEXSpent()
{
	...
	while (true) {
		...
		//Remove spends from zNYEX supply
...
bool RecalculateNYESupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
				...
				if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
					if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
						if (!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
							return state.DoS(100, error("%s : zNyex with serial %s is already in the block %d\n",
	...
	if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
		RecalculateZNYEXMinted();
		RecalculateZNYEXSpent();
		RecalculateNYESupply(1);
	}
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zNyexSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	// If turned on AutoZeromint will automatically convert NYEX to zNYEX
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
	...
	if (miPrev != mapBlockIndex.end()) {
		...
		// nyerium: compute chain trust score
		pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

		// nyerium: compute stake entropy bit for stake modifier
		if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
			LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

		// nyerium: record proof-of-stake hash value
		if (pindexNew->IsProofOfStake()) {
		...
		// nyerium: compute stake modifier
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zNyex mints and %d zNyex spends\n", __func__, nMints, nSpends);

	// nyerium: check proof-of-stake
