...
bool CLegacyZNyexStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZNyexStake from non zc-spend", __func__);
    ...
    *this = CLegacyZNyexStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for znyex stake origin", __func__);

    // All good
    return true;
}

CLegacyZNyexStake::CLegacyZNyexStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZNyexStake::GetIndexFrom()
{
...
CAmount CLegacyZNyexStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZNyexStake::GetUniqueness() const
{
...
bool CLegacyZNyexStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zNYEX stake block: height %d outside range", __func__, nHeight);
