...
#include "qt/nyerium/dashboardwidget.h"
#include "qt/nyerium/forms/ui_dashboardwidget.h"
#include "qt/nyerium/sendconfirmdialog.h"
#include "qt/nyerium/txrow.h"
...
DashboardWidget::DashboardWidget(NyeriumGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareNyex, "square-chart-nyex");
    setCssProperty(ui->labelSquarezNyex, "square-chart-znyex");
    setCssProperty(ui->labelNyex, "text-chart-nyex");
    setCssProperty(ui->labelZnyex, "text-chart-znyex");
    ...
    setCssProperty(ui->labelAmountNyex, "text-stake-nyex-disable");
    setCssProperty(ui->labelAmountZnyex, "text-stake-znyex-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &NyeriumGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        //connect(ui->pushImgEmpty, &QPushButton::clicked, window, &NyeriumGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &NyeriumGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("NYEX")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isNyex = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZNYEX;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair NYEX, zNYEX
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            nyex = (pair.first != 0) ? pair.first / 100000000 : 0;
            znyex = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalNyex += pair.first;
            chartData->totalZnyex += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesNyex.append(nyex);
        chartData->valueszNyex.append(znyex);

        int max = std::max(nyex, znyex);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set1->setColor(QColor(255,225,170));
    ...
    set0->append(chartData->valuesNyex);
    set1->append(chartData->valueszNyex);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalNyex > 0 || chartData->totalZnyex > 0) {
        setCssProperty(ui->labelAmountNyex, "text-stake-nyex");
        setCssProperty(ui->labelAmountZnyex, "text-stake-znyex");
    } else {
        setCssProperty(ui->labelAmountNyex, "text-stake-nyex-disable");
        setCssProperty(ui->labelAmountZnyex, "text-stake-znyex-disable");
    }
    forceUpdateStyle({ui->labelAmountNyex, ui->labelAmountZnyex});
    ui->labelAmountNyex->setText(GUIUtil::formatBalance(chartData->totalNyex, nDisplayUnit));
    ui->labelAmountZnyex->setText(GUIUtil::formatBalance(chartData->totalZnyex, nDisplayUnit, true));
