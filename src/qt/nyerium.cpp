...
#include "qt/nyerium/splash.h"
#include "qt/nyerium/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("nyerium-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in nyerium.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in nyerium.qrc)
...
/** Class encapsulating Nyerium Core startup and shutdown.
...
/** Main Nyerium application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (NyeriumGUI)
...
{
    window = new NyeriumGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &NyeriumGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &NyeriumGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(NyeriumGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NyeriumGUI::DEFAULT_WALLET);
        ...
        // Nyerium: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NyeriumGUI::handlePaymentRequest);
        connect(window, &NyeriumGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Nyerium can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(nyerium_locale);
    Q_INIT_RESOURCE(nyerium);
    ...
    /// 6. Determine availability of data directory and parse nyerium.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Nyerium Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Nyerium Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // nyerium: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Nyerium Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
