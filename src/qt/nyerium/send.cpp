...
#include "qt/nyerium/forms/ui_send.h"
#include "qt/nyerium/addnewcontactdialog.h"
#include "qt/nyerium/qtutils.h"
#include "qt/nyerium/sendchangeaddressdialog.h"
#include "qt/nyerium/optionbutton.h"
#include "qt/nyerium/sendconfirmdialog.h"
#include "qt/nyerium/myaddressrow.h"
#include "qt/nyerium/guitransactionsutils.h"
...
SendWidget::SendWidget(NyeriumGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-nyex");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination nyexAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(nyexAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(nyexAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
