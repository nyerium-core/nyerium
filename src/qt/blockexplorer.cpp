...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::NYEX, nValue);
...
{
    std::string genesisblockhash = "000007ba15bd600e8f0292b6c6becd0f24ffcecadfa2644652324765dcfc0916";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (nyerium.conf).");
            QMessageBox::warning(this, "Nyerium Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
