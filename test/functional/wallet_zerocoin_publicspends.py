...
class ZerocoinSpendTest(NyeriumTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, znyex_bal, nyex_bal):
            znyex_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], znyex_bal)
            nyex_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], nyex_bal)
            return znyex_bal, nyex_bal
        ...
        znyex_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(znyex_balance, 6666)
        ...
        znyex_balance, balance = check_balances(denom_2, znyex_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        znyex_balance, balance = check_balances(denom_3, znyex_balance, balance)
        ...
        _, _ = check_balances(denom_1, znyex_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
