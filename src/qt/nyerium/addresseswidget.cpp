...
#include "qt/nyerium/forms/ui_addresseswidget.h"
#include "qt/nyerium/addresslabelrow.h"
#include "qt/nyerium/addnewaddressdialog.h"
#include "qt/nyerium/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(NyeriumGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination nyexAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(nyexAdd, label.toUtf8().constData(),
