...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
	...
	} else if (nHeight <= 45000 && nHeight > 0) {
		nSubsidy = 200 * COIN;
		...
		nSubsidy = 20 * COIN;
	} else if (nHeight <= 940000 && nHeight > 860000) {
		nSubsidy = 10 * COIN;
	} else if (nHeight > 940000) {
		nSubsidy = 5 * COIN;
	} else {
		nSubsidy = 0 * COIN;
	}
...
bool IsInitialBlockDownload()
{
	...
	bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
		pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60); //EK COMMENT && chainActive.Height() > 3030; // ~144 blocks behind -> 2 x fork detection time
	if (!state)
		lockIBDState = true;
	return state;
}
