...
CzNYEXWallet::CzNYEXWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing znyex seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNYEXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNYEXSeed_deprecated()) {
                    LogPrintf("%s: Updated zNYEX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znyex seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znyex wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNYEXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNYEXCount(nCountLastUsed);
    else if (!walletdb.ReadZNYEXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNYEXWallet::Lock()
{
    seedMaster.SetNull();
}

void CzNYEXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNYEXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNYEX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNYEXWallet::LoadMintPoolFromDB()
{
...
void CzNYEXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNYEXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNYEXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->znyexTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNYEXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNYEX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znyexTracker which also adds to database
    wallet->znyexTracker->Add(dMint, true);
...
void CzNYEXWallet::SeedToZNYEX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzNYEXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNYEXWallet::GenerateDeterministicZNYEX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNYEXWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNYEXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNYEXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
