...
#include "znyex/znyexmodule.h"
#include "znyex/znyexwallet.h"
#include "znyex/znyextracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_Nyerium = 61000, // inherited version..
...
// Possible states for zNYEX send
enum ZerocoinSpendStatus {
    ZNYEX_SPEND_OKAY = 0,                            // No error
    ZNYEX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNYEX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNYEX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNYEX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNYEX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNYEX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNYEX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNYEX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNYEX_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZNYEX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNYEX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNYEX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNYEX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNYEX_SPENT_USED_ZNYEX = 14,                      // Coin has already been spend
    ZNYEX_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZNYEX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 25000 NYEX output and keys which can be used for the Masternode
    ...
    void doZNyexRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZNYEX);
    ...
    bool CreateZNYEXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zNYEX wallet
    CzNYEXWallet* zwalletMain{nullptr};
    std::unique_ptr<CzNYEXTracker> znyexTracker{nullptr};
    void setZWallet(CzNYEXWallet* zwallet);
    CzNYEXWallet* getZWallet();
    ...
    // zNYEX reset
    boost::signals2::signal<void()> NotifyzNYEXReset;
