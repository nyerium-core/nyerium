...
The build script we'll be using is contained in the Nyerium github repository ([contrib/gitian-build.py](https://github.com/nyerium-core/nyerium/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Nyerium repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/nyerium-core/Nyerium/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, nyerium-detached-sigs, and nyerium GitHub repos.
