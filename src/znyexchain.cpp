...
{
    CDataStream serializedCoinSpend = ZNYEXModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZNYEXSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zNYEX supply (mints are forever disabled after last checkpoint)
...
bool UpdateZNYEXSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        ...
        for (const CZerocoinMint& m : listMints) {
            ...
            // zerocoin failsafe
            if (mapZerocoinSupply.at(denom) < 0)
                return error("UpdateZNYEXSupplyDisconnect: Block contains zerocoins that spend more than are in the available supply to spend");
        }
    }
