...
class ReorgStakeTest(NyeriumTestFramework):
    ...
    def check_money_supply(self, expected_nyex, expected_znyex):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected NYEX and zNYEX supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_nyex))
            for denom in node['zNYEXsupply']:
                assert_equal(node['zNYEXsupply'][denom], DecimalAmt(expected_znyex[denom]))
