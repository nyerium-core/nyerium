                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNYEX ought to be enough for anybody." - Bill Gates, 2017
    ui->zNYEXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNYEXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNYEX->setEnabled(false);
        ui->pushButtonMintzNYEX->setToolTip(tr("zNYEX is currently disabled due to maintenance."));

        ui->pushButtonSpendzNYEX->setEnabled(false);
        ui->pushButtonSpendzNYEX->setToolTip(tr("zNYEX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNYEXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNYEX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNYEX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNYEX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNYEX in ") +
...
void PrivacyDialog::on_pushButtonSpendzNYEX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNYEX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNYEX
        sendzNYEX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNYEX
    sendzNYEX();
}

void PrivacyDialog::on_pushButtonZNyexControl_clicked()
{
    ZNyexControlDialog* zNyexControl = new ZNyexControlDialog(this);
    zNyexControl->setModel(walletModel);
    zNyexControl->exec();
}

void PrivacyDialog::setZNyexControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNyexSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNYEX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Nyerium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNYEXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNYEX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NYEX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNYEX</b>";
    ...
    // use mints from zNyex selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNyexControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNyexControlDialog::GetSelectedMints();
    }
    ...
    // Clear znyex selector in case it was used
    ZNyexControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNyex Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNYEX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNYEX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNYEX "));
    ui->labelzNYEXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNYEXSyncStatus->setVisible(fShow);
}
