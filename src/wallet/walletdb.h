...
public:
    ...
    bool WriteZNYEXSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZNYEXSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZNYEXSeed_deprecated(uint256& seed);
    bool EraseZNYEXSeed();
    bool EraseZNYEXSeed_deprecated();

    bool WriteZNYEXCount(const uint32_t& nCount);
    bool ReadZNYEXCount(uint32_t& nCount);
