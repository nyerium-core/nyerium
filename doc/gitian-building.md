...
*Setup instructions for a gitian build of Nyerium using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nyerium
...
- [Building Nyerium](#building-nyerium)
...
Clone the git repositories for nyerium and gitian and then checkout the nyerium version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nyerium repository
...
To build Nyerium (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the nyerium repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nyerium/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/nyerium-core/nyerium.git
...
./bin/gbuild --commit nyerium=${COMMIT} --url nyerium=${URL} ../Nyerium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nyerium=${COMMIT} --url nyerium=${URL} ../Nyerium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nyerium=${COMMIT} --url nyerium=${URL} ../Nyerium/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nyerium-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nyerium-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nyerium-build.assert
```
...
[nyerium-core/gitian.sigs](https://github.com/nyerium-core/gitian.sigs/) repository, or if that's not possible to create a pull
request.
